...
const Create = () => {
  ...
  useEffect(() => {
    ...
    Flatpickr('#start-time', {
      ...
      onChange: (selectedDates, dateStr) => {
        ...
        endTime._flatpickr.set('minTime', dateStr);
      }
    });
    ...
    Flatpickr('#end-date', {
      ...
      onChange: (selectedDates, dateStr) => {
        ...
        startDate._flatpickr.set('maxDate', dateStr);
      }
    });
    ...
    Flatpickr('#end-time', {
      ...
      onChange: (selectedDates, dateStr) => {
        ...
        startTime._flatpickr.set('maxTime', dateStr);
      }
    });
  }, []);
  ...
  const descMaxChars = 5000;
  const ticketDescMaxChars = 100;

  const updateTicket = (index, field, value) => {
    const newTickets = [...tickets];
    
    if (field === 'description' && value.length > ticketDescMaxChars) {
      value = value.slice(0, ticketDescMaxChars); // Enforce character limit
    }
  ...
  const handleDescChange = (e) => {
    if (e.target.value.length <= descMaxChars) {
  ...
  const handleEventCreation = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        router.push(`/dashboard/events/preview/${eventId}`);
      }
    } catch (error) {
      console.error('Error creating event:', error);
      toast.error(error.response.data || 'Error creating event. Please try again.');
    }
  };
        ...
        <div className="w-full border-b border-b-gray-200 flex gap-4 pb-5 justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button 
              className="border border-violet-600 rounded-lg shadow py-2.5 px-4 bg-violet-600 h-fit font-semibold text-sm text-white"
              onClick={() => {
                // If the current page is 1, check if name, description, imageurl, category, location, timezone, frequency(if selected is recurring, else set frequncy to single), startdate, enddate, starttime, endtime are not empty
                if (page === 1) {
                  if (name === '') {
                  ...
                  // If the current page is 2, check if at least one ticket is added
                  if (tickets.length === 0) {
                    return toast.error('Please add at least one ticket');
                  }

                  if (purchaseLimit === '') {
                    return toast.error('Please set a purchase limit');
                  }

                  // Check if all tickets have a name, description, quantity and price (if type is paid)
                  const invalidTickets = tickets.filter((ticket) => {
                    return ticket.name === '' || ticket.description === '' || ticket.quantity === '' || (ticket.type === 'paid' && ticket.price === '');
                  });
              ...
              <div className="w-2/4 flex flex-col gap-3">
                ...
                <div className="w-full flex flex-col gap-1.5">
                  <textarea 
                    ...
                    required
                  ></textarea>
                  <p className="font-normal text-sm text-gray-600">{descMaxChars - description.length} characters left</p>
                </div>
              </div>
            </div>
            <div className="w-full h-[1px] bg-gray-200" />
              ...
              <div className="flex flex-col w-2/4 gap-5">
                ...
                {selected === 'recurring' && (
                  <select 
                    ...
                    {...(selected === 'recurring' ? { required: true } : {})}
                  >
                ...
                <div className="flex flex-col gap-1.5 w-full">
                  ...
                  <div className="flex gap-4 w-full">
                    <div className="border border-gray-300 w-full shadow py-2.5 px-3 rounded-lg gap-2 flex items-center">
                      ...
                      <input type="text" placeholder="yyyy-mm-dd" id="start-date" className="w-full outline-none text-gray-500 text-base font-normal" value={startdate} />
                    </div>
                    <div className="border border-gray-300 w-full shadow py-2.5 px-3 rounded-lg gap-2 flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 text-gray-500">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                      </svg>
                      <input type="text" placeholder="hh:mm" className="w-full outline-none text-gray-500 text-base font-normal" id='start-time' value={starttime} />
                    </div>
                  </div>
                </div>
                <div className="flex flex-col gap-1.5 w-full">
                  ...
                  <div className="flex gap-4 w-full">
                    <div className="border border-gray-300 w-full shadow py-2.5 px-3 rounded-lg gap-2 flex items-center">
                      ...
                      <input type="text" placeholder="yyyy-mm-dd" id="end-date" className="w-full outline-none text-gray-500 text-base font-normal" value={enddate} />
                    </div>
                    <div className="border border-gray-300 w-full shadow py-2.5 px-3 rounded-lg gap-2 flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 text-gray-500">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                      </svg>
                      <input type="text" placeholder="hh:mm" className="w-full outline-none text-gray-500 text-base font-normal" id='end-time' value={endtime} />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex gap-8">
                  ...
                  <div className="w-2/4 flex flex-col gap-3">
                    ...
                    <div className="w-full flex flex-col gap-1.5">
                      ...
                      ></textarea>
                      <p className="font-normal text-sm text-gray-600">{ticketDescMaxChars - ticket.description.length} characters left</p>
                    </div>
                  </div>
                </div>
                <div className="w-full h-[1px] bg-gray-200"/>
